...
$$
\vert 0\rangle^{\otimes n} \rightarrow{R_Y(x_i)} \rightarrow{[\text{Rot}(\theta,\phi,\lambda) + \text{CNOT}] \times L} \rightarrow{\text{Rot}(\theta,\phi,\lambda)} \rightarrow{\langle Z_i \rangle}
$$
